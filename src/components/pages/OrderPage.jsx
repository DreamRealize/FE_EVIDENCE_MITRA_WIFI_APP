...
const OrderPage = ({
   ...
   const totalItems = orderData?.pageInfo?.total || 0; // Total jumlah data
   ...
   //    orderRequest(1); // Panggil ulang data untuk halaman pertama
   // }, [orderData]); // Pantau perubahan data pemasukan
   ...
   const columns = [
      ...
      {
         title: "Date",
         dataIndex: "waktuOrder",
         render: (waktuOrder) => (
            <span>{new Date(waktuOrder).toLocaleDateString("id-ID")}</span>
         ),
         sorter: {
            compare: (a, b) =>
               new Date(a.WaktuPemasukan) - new Date(b.WaktuPemasukan),
               ...
               <Row justify="space-between" style={{ padding: "16px 0" }}>
                  ...
                  <Col span={12} style={{ textAlign: "right" }}>
                     <CustomPagination
                        ...
                        paging={orderData?.pageInfo?.page}
                     />
                  </Col>
               </Row>
               <Table
