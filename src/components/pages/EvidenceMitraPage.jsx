...
const EvidenceMitraPage = ({
	...
	const [filterModalVisible, setFilterModalVisible] =
		useState(false);
	const [columnModalVisible, setColumnModalVisible] =
		useState(false);
	const [deleteModalVisible, setDeleteModalVisible] =
		useState(false);
	const [selectedPemasukanID, setSelectedPemasukanID] =
		useState(null);
	const [currentPage, setCurrentPage] = useState(1);
	...
	const columns = [
		...
		},
		{
			...
			render: (waktuEviden) => (
				<span>
					{new Date(waktuEviden).toLocaleDateString(
						"id-ID"
					)}
				</span>
			),
			sorter: {
				compare: (a, b) =>
					new Date(a.WaktuPemasukan) -
					new Date(b.WaktuPemasukan),
			},
		},
		{
			title: "Service",
		...
		},
		{
			...
			render: (statusOrder) =>
				statusOrder?.statusName || "-",
		},
		{
			title: "Status Eviden",
			dataIndex: "statusEviden",
			render: (statusEviden) =>
				statusEviden?.evidenceName || "-",
		},
		{
			title: "Note",
